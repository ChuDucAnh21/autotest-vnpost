...
test.describe("Kiểm tra chức năng Tạo CTKM", () => {
  ...
  test("CTKM_21: Khai báo giảm giá bán theo sản phẩm cụ thể (giảm %)", async ({
    page,
  }) => {
    ...
    const ctkmName = `CTKM_ProductPercent_${Date.now()}`;
    ...
    // Cấu hình Điều kiện 1: Mua từ 3 -> Chọn loại "Sản phẩm" -> Click ô tìm -> Chọn SP -> Giảm 15 %
    ...
    await page.locator(".ant-input-number-input").nth(1).fill("15");
